...
- **Phase 06 — Two-Stage Search Provider (DEFERRED post-MVP, Session 15 decision):** The original "Claude-native" direction was rejected Session 15 — Anthropic API `web_search` costs ~$10/1k (more than Exa's $7/1k), and headless Claude Code under the Max plan is viability-risky (`claude -p` expected to draw user credits, not plan allowance). Replacement design researched and recorded in `project-management/phase-plan-notes/phase-06/phase-06-search-provider-decision.md`: Serper SERP (~$1/1k, Google-quality) + the user's own crawl4ai Railway service (BM25 content filtering, $0 marginal) → Evidence. Deferred until time allows; a control-hub IDEA note proposes generalizing the capability inside the crawl4ai repo for other projects. **Interim: user buys Exa credits; Tavily free tier (1k/month) as fallback.** Evaluation plan when built: goal-loop against the 5-claim comparator set (`project-management/phase-plan-notes/phase-06/phase-06-comparator-set.md`), unchanged.
...
**Session 14 prep (2026-07-25):** Comparator set gathered — 5 claims from ukraine-intro-test.txt searched through Exa (all 402, still dead) and Tavily (15 results). Baseline data in `project-management/phase-plan-notes/phase-06/phase-06-comparator-set.md`. Integration surface mapped (Code Explorer). Three evaluation metrics defined: speed, quality/accuracy, cost. Orchestration approach decided: Fable + goal-loop pattern for experimental implementation.

**Session 15 research (2026-07-25):** Claude-native direction rejected (D60). Market scan of 7 providers. PixSerp discovered and evaluated (32 live API calls: 5-claim comparator + 27-call matrix across tiers/prompts). Key finding: PixSerp handles specific-entity queries (claim 2) well at $1.50/1k but synthesized answers hallucinate (claim 5) and refutation is weaker than Tavily (claim 3). Cannot replace Tavily's full-page depth alone. Query-design playbook written (`docs/playbook/query-design.md`). crawl4ai service explored and documented. User decision: buy Exa credits for interim; Phase 06 implementation deferred post-MVP. Full decision record: `project-management/phase-plan-notes/phase-06/phase-06-search-provider-decision.md`.
